...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 15-09-2021 a las 17:13:44
...
-- Funciones
--
CREATE DEFINER=`root`@`localhost` FUNCTION `fun_agregar_palabra` (`p_palabra` VARCHAR(255), `p_id_tipo` INT, `p_id_idioma` INT) RETURNS BIT(1) BEGIN 
  DECLARE v BIT;
...
(23, 'T1', 'D4', 'SEGURIDAD'),
(24, 'P1', 'D41', 'El mecanismo de seguridad que se usará, será por medio de una encriptación, para evitar que sea hurtada la información del contenido del software.'),
(25, 'P1', 'D41', 'Semanalmente se realizará una copia de seguridad, esto para evitar perdida de la información en caso de actualizaciones o cambios del mismo.'),
(26, 'P1', 'D41', 'Si presentan inconsistencias del software, se podrá realizar una restauración del contenido para reparar las anomalías.'),
(27, 'P1', 'D41', 'Estará alojado en el servidor de hosting, para que tenga el privilegio de la realización de las copias de seguridad.'),
(28, 'P1', 'D41', 'Toda imagen usada por el traductor, es de autoría de una de sus creadoras (Lizbeth Caro)'),
(29, 'T2', 'D412', 'LICENCIA'),
(30, 'P1', 'D4122', 'El copyright representa los derechos de autor que se le conceden a las creadoras del proyecto Traductor Nativo'),
(31, 'T1', 'D5', 'FORTALEZAS'),
(32, 'P1', 'D51', 'Desde los inicios de este proyecto, siempre se tuvo claro el tema central, enfocándonos en una de las necesidades que presenta la etnia indígena de panore en San José del Guaviare.'),
(33, 'P1', 'D51', 'Además de que una de las organizadoras y creadoras del proyecto es perteneciente de la etnia y tiene conocimiento de la lengua.'),
(34, 'P1', 'D51', 'Durante el proceso de este producto, se afianzaron temas y conocimientos, esto nos permitió realizar mejorías para un mejor desarrollo de nuestro proyecto, dando como resultado un excelente proceso y un terminado completo.'),
(35, 'T1', 'D6', 'DEBILIDADES'),
(36, 'P1', 'D61', 'La lengua del tucano ha perdido fuerza, y esto se debe a que muchos de los que pertenecen a la etnia, se han civilizado, y los que tienen un dominio de su propia lengua han venido falleciendo, dificultando la recolección de datos para seguir alimentando la base de datos. '),
(37, 'P1', 'D61', 'En cuanto a la programación y tecnologías, cada día van innovando, esto obliga a conocer diferentes formas de programar y conocer nuevas líneas de codificación, debilitando el proceso de mejoras del mismo.'),
(39, 'IM', '', 'https://www.tooltyp.com/wp-content/uploads/2014/10/1900x920-8-beneficios-de-usar-imagenes-en-nuestros-sitios-web.jpg');
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vista_idiomas`  AS SELECT count(0) AS `conteo`, `t1`.`idioma` AS `idioma` FROM `idiomas` AS `t1` WHERE `t1`.`id` <> 0 GROUP BY `t1`.`idioma` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vista_palabras`  AS SELECT count(0) AS `conteo`, `t1`.`palabra` AS `palabra` FROM `palabras` AS `t1` WHERE `t1`.`id` <> 0 GROUP BY `t1`.`palabra` ;
